...
export const Navbar = () => {
  ...
  const { logout, loading, authUser } = useAuthContext();
      ...
      <>
        <div className="sticky top-0 left-0 right-0 z-50 border-b bg-background">
          ...
          <div
              ref={menuRef}
              className={cn(
                  "fixed top-0 right-0 w-4/5 h-screen overflow-hidden bg-background transition-transform duration-300 ease-in-out z-20 shadow-2xl",
              ...
              {authUser && (
                  <>
                    ...
                    <Button
                        ...
                        variant="link"
                        disabled={loading}
                        className="flex items-center justify-start gap-4 text-destructive"
                    >
                      <LogOut className="w-4 h-4" />
                      {t("navbar.logout")}
                    </Button>
